<%@ page language="java" contentType="text/html; charset=UTF-8"
	...
	<%Vector<Article> vlistArticle = (Vector<Article>)session.getAttribute("listA"); 
...
<body>
	...
	<!-- End HEADER -->
		<%
		if (vlistArticle != null || request.getAttribute("noOfPages") != null
    ...
    <div class="container">
       ...
       <div class="list-group-item" style="background-color: #228B22">
						...
						<div class="col-xs-12">
							<div class="col-xs-8">
								<h2 class="font-size-20 bold">
									<font color="#FFFFFF">บทความทั้งหมด
									</font>
								</h2>
							</div>
						</div>
		...
		<br>
		<%try{ %>
	       <%for(Article av: vlistArticle){ %>
			  <!-- Project One -->
	        <div class="row">
	            <div class="col-md-5">
	                <a href="#">
	                    <img class="img-responsive" src="Tools/images/article/<%=av.getVectorimages().get(0).getImageName()%>" alt="image2" width="700px" height="400px">
	                </a>
	            </div>
	            <div class="col-md-6">
	                <h3><%=av.getArticleName() %></h3>
	                <h4>จำนวนคนเข้าชม : <%=av.getStatisticsvisit() %></h4>
	                <p><%=av.getArticleTitel() %></p>
	                <p><div class="fb-like" data-href="<%=url%>pongyeang/ViewArticlesServlet?va=<%=av.getArticleID()%>" 
					data-width="500" data-layout="standard" data-action="like" data-size="large" data-show-faces="true" data-share="true"></div></p>
	                <a class="btn btn-primary" href="ViewArticlesServlet?va=<%=av.getArticleID()%>">อ่านต่อ  <span class="glyphicon glyphicon-chevron-right"></span></a>
	            </div>
	        </div>
	        <hr>
	        <%} %>
			...
			<div class="row">
				<div class="col col-xs-offset-3 col-xs-6">
					...
					<ul class="pagination">
						...
						%>
						<li><a
							href="ListArticleServlet?page=<%=currentPage - 1%>"
							aria-label="Previous"><span aria-hidden="true">«</span></a></li>
						<%
							for (int s = 0; s < noOfPages; s++) {
						%>
						<li><a href="ListArticleServlet?page=<%=s + 1%>"><span
